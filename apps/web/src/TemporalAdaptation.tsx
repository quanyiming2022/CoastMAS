...
function TemporalForm({ projectId }: { projectId: string }) {
  ...
  return (
    <>
      ...
      <fieldset className="form-workspace form-shell" disabled={save.isPending}>
        <Panel title="来源与场景">
          <label>
            沿用研究区的场景
            <select
          ...
          <div className="form-grid">
            {(
              ...
              <label
                key={name}
                className={name === "name" ? "form-full" : undefined}
              >
                {label}
                <input
                  value={fields[name]}
        ...
        <Panel title="变量与单位">
          <div className="form-grid">
            {(
              ...
              <label
                key={name}
                className={name === "variable" ? "form-full" : undefined}
              >
                {label}
                <input
                  value={fields[name]}
        ...
        <Panel title="目标时间与观测">
          ...
          <DataTable aria-label="时间观测" className="observation-table">
            <thead>
              <tr>
                <th>{actualSupport === "point" ? "观测时刻" : "区间开始"}</th>
                {actualSupport === "interval" ? <th>区间结束</th> : null}
                <th className="numeric">数值</th>
              ...
              {observations.map((row, index) => (
                <tr key={index}>
                  {(
                    [
                      "start",
                      ...(actualSupport === "interval" ? ["end" as const] : []),
                      "value",
                    ] as const
                  ).map((field) => (
                    <td
                      key={field}
                      className={field === "value" ? "numeric" : undefined}
                    >
                      <input
