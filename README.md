...
代码仓库：<https://github.com/quanyiming2022/CoastMAS>（`main`）。当前新实现与启动说明见 [next/README.md](next/README.md)；本次验收、能力边界与问题见 [.review/acceptance-report.md](.review/acceptance-report.md)。后续任务独立提交并普通推送，交付 before/after commit；不改写审查历史。
...
| 任务 | 操作与限制 |
|---|---|
| 用户操作 | [详细用户说明书](docs/user-manual-detailed.md)（字段、按钮、表格与操作示例） · [简明版](docs/user-manual.md) |
| 本机一键启动 | [启动脚本](start-coastmas.command)（先打开 Docker Desktop，双击或终端运行） |
...
| 本地真实业务资料 | [data_quan 接入状态与用法](docs/business-data-integration.md)（资料检查已实现，科学计算待确认） |
