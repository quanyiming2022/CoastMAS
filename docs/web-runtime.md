...
登录后选择项目，打开工作流 `coastal_impact`，选择 `Synthetic coastal inundation screening`，执行科学预检后提交。运行中心显示真实进度，成功结果可下载完整 JSON 并查看来源、地形筛查多边形和分区人口统计。合成例金标准为 80000 m²、320 人；人口按完整管理单元均匀分布，不是精细人口分布或水动力模拟。其他必选页面、全部编辑与研究功能仍在研发。
...
当前10项组件/契约测试、2项真实浏览器测试覆盖已实现纵向链路，不代表全部操作或完整产品验收。前端包体仍存在地图/图表大块警告，相关模块按需加载；未修改阈值掩盖警告。
