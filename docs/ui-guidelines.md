...
当前V3开发工作台以 `next/web/src/navigation.ts` 和 `V3Navigation.tsx` 为唯一导航来源：工作台、项目与研究、数据、指标与方法、模型工程、耦合与工作流、规划与优化、运行、成果、管理。项目CRUD归项目与研究，管理不复制项目真源；横切能力放既定详情，未实现入口仅开发环境显示禁用状态。四类任务八步由 `taskFlows.ts` 分别定义，未接通阶段如实标注。

当前桌面视觉tokens集中于 `next/web/src/desktop-theme.css`：中性工具窗格、teal强调、13px基础字体、32px普通控件、28–34px表格行、2/4/6px圆角与4/8/12/16间距。共享Button/Input/Select/Tabs/Table/Inspector/StepRail/Dock延续现有语义；不以样式改变运行/科学状态。桌面验收覆盖1366/1440/1920/2560，实际证据见next的当前STATE。下列旧版规则与截图保留为历史记录，不约束当前V3菜单和视觉尺寸。
...
## 新版专业工作台（2026-09-24，独立版基线）

`next/web/src/workbench.css`集中维护48px顶栏、56/208px导航轨、左目录与右侧属性宽度、32px工作台控件及局部滚动。旧版兼容约束不适用于next实现。工作台画布按1440/1366实测宽≥680px且≥应用宽50%、高≥可用高60%验收；手机主视图优先，配置收进切换区。表格普通输入宽度规则必须排除checkbox/radio，选择命中区独立；不通过强制点击掩盖遮挡。

原件名称/显示名称、科学版本/目录修订、输入/实际成果分别表达。搜索针对服务端全集，中文输入完成后300ms防抖；空数据、无匹配与请求失败分开。批量操作先冻结选择、展示影响，再执行逐项重验；仅响应已确认时显示已保存/已完成。当前资产目录与有效覆盖链先执行这些规范，尚未据此宣称全站已覆盖。

## 当前研究面板与地图显示裁决（2026-09-24）

本节替代上节默认图标轨、顶部横排步骤及固定宽度占比要求。布局保留完整中文主菜单、48px合并顶栏、260px研究内容面板、72px右侧流程栏和按需展开的320px互斥面板；地图使用剩余空间，尺寸变化只resize，不重新定位。

图层、输入、运行分标签；搜索不影响显隐，选择不改变输入，移出地图不删除资产。运行显示实际时间、配置版本和中文状态，计算成功不等于业务验证通过。

右侧两字标签：资料、对齐、落图、指标、标化、权重、综合、验证；完整含义依次为资料装配、空间统一、统计空间化、指标生产、指标标准化、权重配置、综合计算、验证与交付。悬停、键盘焦点及步骤面板保留完整名称和状态。点击不执行、不绑定、不新建任务；步骤、详情、样式、点查共用右侧位置。

地图图例默认关闭，在样式中开关并保存个人偏好；只对应活动可见图层。普通连续量190px紧凑图例；真实单值画单色块并标注预览样本，近值按必要精度区分，分类可滚动查看全部条目。完整依据留在样式/详情。加载、显示错误及重试在工具条，地图内无保存横幅。顶部合并呈现保存状态，内部区分视图和研究配置；历史运行不因草稿保存而重算。工程结果待验证限制保留，网页图例开关不修改文件或科研图件模板。
