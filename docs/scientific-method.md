...
`coastal_catalog` 注册前运行合成样例金标准。screening、overlay、statistics 分别处理连通筛查、显式人口分配和管理单元统计。人口分母为完整管理单元格网，不因研究区裁剪而改变；管理单元超出格网覆盖时拒绝计算。缺失高程的面积和人口单列为未知，不推断为安全。

目标格网必须可逆、至多 400 万像元，并与模型允许的 CRS 相容。人口面积计算要求投影格网。真实三节点 DAG 的证据为 `artifacts/evidence/20260920T101645146099Z-coastal-dag-full.json`；这还不是包含规划、API、worker 和界面的完整场景验收。
