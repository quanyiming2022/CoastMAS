...
- 最近已保存提交 `116c344`（科学算法、数据库、API、worker）；本文件描述其后同批适配器改动，最新提交以 git log -1 为准。
...
- 最新统一测试：`20260920T081955993543Z-measured-subprocesses`，131 passed，2 项依赖弃用警告。类型/lint `20260920T082004*` 通过。软件包可安装并从 /tmp 导入，pip check 通过。
- 覆盖率：行 87.8159%、分支 65.3191%；业务分支及核心门槛尚未通过。不得宣称完整质量门禁通过。
...
- 接下来：模型拆解与检索，注册内置模型并把三场景拆解为可规划 DAG；规划/图谱/实体/数据导入；完整 API/UI；协同、动态评价和科研页面；契约生成/全门禁/最终文档与统一验收。
