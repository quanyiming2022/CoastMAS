...
## 科研任务与本地提供方检查点
- 影像/管理/隔离测试已提交a766e78；证据数值预算误报修复ab84d08。之前提交扫描唯一命中是LLM_MAX_COMPLETION_TOKENS的数字2048，并非秘密；已限定排除该配置，真实凭证仍脱敏，两个提交重新扫描通过。
- 科研A/B/C共享固定目录，C使用实际CAN_FOLLOW图谱证据；来源区分RULE/LOCAL/EXTERNAL/MOCK/REPLAY。候选完整性/目录/科学预检拒绝保留原建议，未知人工修订不补0；真实请求来自dispatched_at，不用预算预留冒充。
- 同一Job租约、取消、权限复核和不可变ResultBundle新增research_evaluation分支；明确不执行候选。重试保持evaluation_id，已有试验账本不重复请求，无法重放的已发送请求记录失败。结果标RAW，任务SUCCEEDED与评估COMPLETE/PARTIAL/BLOCKED分别显示。
- API/worker/原运行接口14项局部回归063458672417Z通过；前端47项、类型/lint/构建064036976327Z通过；真实研究浏览器064307985845Z通过。全后端与19条浏览器正在执行，未提前记通过。
- 本机Qwen实际A/B/C对照064514959214Z通过执行取证；A有效，B MODEL_UNKNOWN、C PLAN_INCOMPLETE，两候选均被阻止。B/C分别2596/2949实际tokens，总5545；只1个固定样例，不能推断总体优劣或宣称规划质量达标。报告planning-80bd6c0b-44c7-4762-aa45-c61a59be74e9.json，实际账本留在归档合成验证项目。
- 服务API39484、worker39494加载科研功能，日志api/worker-research-interface.log；beat33314仍运行。默认真实影像项目未加入合成样例。外部云实验仍BLOCKED；人工修订观测入口、完整错误注入/消融/性能实验及R68十二项统一科研报告仍未完成。

- 本轮完整后端064524240630Z通过400项、11警告，完整隔离浏览器064753734560Z通过19条；临时资源已清理。科研截图已查看，显示部分完成/明确分母/未观测，未假报B/C成功。当前无验收进程。下一步补齐人工修订观测、绑定注入/消融/性能、部署验收与已列必选缺口。

## 当前安全点与追加UI要求（2026-09-21）
- 时间适配/cubic已提交c42b0d9；完整后端073226129507Z为421 passed，完整浏览器073247442736Z为20 passed；追加边界25项/前端51项/最终时间UI074116104778Z通过。
- 覆盖率artifacts/coverage-temporal.json：全体行89.44%/分支73.02%，核心行91.17%/分支78.86%；分支门槛未过。
- 用户新增UI要求前已发起的服务更新完成：API44796、worker44797、beat33314；日志api-temporal.log/worker-temporal.log，ready通过。UI工作不得停止它们。
- 当前安全点开始全站UI一致性修复，限主题/共享组件/布局/展示标记/测试；不改API、科学算法、业务提交、权限及任务逻辑。保留其他既有截图改动。默认单执行者。
- 原因：main.tsx样式正常加载，但form-grid未定义；裸全局控件规则影响范围过大；协同目录缺空态、表格滚动容器、比较/分页分组。
- 验收要求：1440/1366/390三个视口，代表页空态/填表态同尺寸前后截图，查看实际截图；请求字段和值对照、键盘/禁用/选择/分页/提交及全站覆盖，地图/工作流不受污染。
- UI完成后继续原任务：科研人工修订观测/错误注入/消融/性能/十二项报告、跨CRS守恒、可信运行审批、任务诊断/缓存/保留清理、可选GeoAI接口、部署入口/构建身份、最终门禁。不得把UI完成当全项目完成。
- 已复核需求未限定云端提供方；真实本机Qwen可用于B/C对照，云端来源未配置单列BLOCKED而非本机研发前置条件；来源不能混淆。

## NAV-01 安全点
- 单一navigation配置覆盖原16个侧栏入口；模型中心/知识图谱真实存在，数据来源/评价记录/模型拆解保留原页面内入口并正确归属。仅改导航与展示名称，未改API/科学/权限/退出事件，未重启主服务。
- 080722394968Z：前端55项、lint、类型构建通过；浏览器地图测试误用无几何实体页失败，已保存失败证据并改为真实种子场景地图。
- 080914753753Z：导航4项单测、lint、类型构建、真实浏览器2项通过；管理员16入口/普通账号15入口；直达、前后退、刷新、存储失败、上下文、表单与地图/工作流DOM保留、抽屉焦点及Esc均验证。
- 前后截图artifacts/navigation/{before,after}，1440x900/1366x768/390x844；追加1366x360短窗口及长邮箱截图已实际审阅。无伪造入口；地图/ECharts大包构建提示仍保留。
- 接下来继续先前追加的全站UI统一：共享控件/Table/Form、协同空态分页、时间表单分组、全页面覆盖与提交对照。UI完成后按上节继续原研发，整体未完成。

## UI/NAV最终检查点（2026-09-21）
- NAV首次提交b60feba后，完成共享样式、33处DataTable、表单双列/分区、协同空态与比较/分页、列表加载/空态/错误区别。所有实际请求字段和值与改造前一致（包括0和null）；主科研数据未写入。
- 085413062004Z-ui-visual-final：58项前端单测、lint、类型/构建、24条隔离真实浏览器全部通过。覆盖42条现有列表/编辑/详情路径，1440x900/1366x768/390x844，共126张覆盖截图；无持续整页溢出、按钮重叠、主链接按钮对比度不足。
- 084726416643Z另含10项前端文档分发测试通过；084019091881Z含对应ruff/mypy通过。NAV末次修复保证手动分组开合不拉回滚动位置；鉴权/权限/退出处理未改。
- 已实际查看导航、协同、时间适配、管理、结果对照、科研报告、数据来源、地图及工作流截图。代表页前后同尺寸图见artifacts/ui-consistency/{before,after}；NAV前后图见artifacts/navigation/{before,after}；全页面见artifacts/ui-consistency/coverage。原平铺导航不存在折叠态，折叠图仅适用于改造后。
