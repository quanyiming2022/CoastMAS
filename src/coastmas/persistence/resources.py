...
from coastmas.persistence.schema import AuditLog, Membership, Resource, ResourceVersion, User
...
def update_resource(
    ...
    *,
    ...
    resource = session.scalar(select(Resource).where(Resource.id == identifier).with_for_update())
    if resource is None:
        raise CoastMASError("AUTHORIZATION_ERROR", "permission denied or resource unavailable")
