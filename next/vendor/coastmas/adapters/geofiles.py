    ...
    *,
    ...
    try:
        with MemoryFile(content) as memory, memory.open(driver="GTiff") as dataset:
            ...
            tags = dataset.tags()
            unit = dataset.units[0] or dataset.tags(1).get("unit") or tags.get("unit")
            if unit and declared_unit is not None and unit != declared_unit:
                raise ConstraintError("file and declared unit differ")
            unit = unit or declared_unit
            if dataset.scales[0] != 1 or dataset.offsets[0] != 0:
                raise ConstraintError("nonidentity scale/offset requires explicit mapping")
